...
- [ ] `get_view_list` returns all views, respects type filter
- [ ] `switch_view` changes active view by ID
- [ ] `open_view` finds and opens view by name with partial matching
- [ ] `create_floor_plan_view` creates plan for specified level
- [ ] `create_3d_view` creates 3D view with orientation presets
- [ ] `create_section_view` creates section at specified location/direction
- [ ] `create_schedule_view` creates schedule with specified fields
- [ ] `create_drafting_view` creates blank drafting view
- [ ] All view creation tools integrate with undo (single Ctrl+Z)

...
|-----------|-------|--------|-------------|
| `isolate_elements` | `element_ids: int[]`, `temporary?: bool` | Success | Yes |
| `hide_elements` | `element_ids: int[]`, `temporary?: bool` | Success | Yes |
| `reset_visibility` | `temporary_only?: bool` | Success | Yes |
| `set_3d_section_box` | `element_ids?: int[]` or bounds | Success | Yes |
| `clear_section_box` | - | Success | Yes |
| `set_display_style` | `style: string` | Success | Yes |

**Note**: All visibility operations require transactions in Revit 2026, including temporary hide/isolate modes.
...
1. **Temporary Hide/Isolate** (`TemporaryViewMode`) - UI-level, not saved, easily reset with `DisableTemporaryViewMode`
2. **Permanent Override** - Saved with view, uses `HideElements`/`UnhideElements`

**Important (Revit 2026)**: Both temporary and permanent visibility operations require an active transaction. The `RequiresTransaction = true` flag is set on all visibility tools.

```csharp
// Both modes require transaction in Revit 2026
...
Before hiding/isolating elements, always check:
- `element != null` - Element exists in document
- `element.CanBeHidden(view)` - Element can be hidden in this view (some elements like levels, grids cannot be hidden)
...
- [x] `isolate_elements` shows only specified elements (temporary mode)
- [x] `isolate_elements` with temporary=false saves isolation with view
- [x] `hide_elements` hides specified elements while showing others
- [x] `hide_elements` with temporary=false saves with view
- [x] `reset_visibility` clears temporary overrides
- [x] `reset_visibility` with temporary_only=false clears permanent overrides
- [x] `set_3d_section_box` with element_ids creates appropriate box with padding
- [x] `set_3d_section_box` with min/max creates exact box
- [x] `clear_section_box` removes section box
- [x] `set_display_style` applies all listed styles
- [x] Error messages are helpful when operations fail

...
- [ ] `zoom_to_fit` shows entire model in view
- [ ] `zoom_to_elements` centers and fits specified elements
- [ ] `zoom_to_bounds` zooms to exact coordinate region
- [ ] `zoom_by_percent` zooms in (>100) and out (<100) correctly
- [ ] `pan_view` moves view by direction and distance
- [ ] `pan_view` centers on specified element
- [ ] `pan_view` centers on specified coordinates
- [ ] `orbit_view` rotates 3D view horizontally and vertically
- [ ] `orbit_view` errors gracefully on 2D views
- [ ] `set_view_orientation` applies all preset orientations
- [ ] All tools work in floor plans, elevations, sections, and 3D views (where applicable)

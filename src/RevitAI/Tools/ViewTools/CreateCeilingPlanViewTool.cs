...
/// <summary>
/// Tool that creates a new ceiling plan view for a specified level.
/// </summary>
public sealed class CreateCeilingPlanViewTool : IRevitTool
{
    private static readonly JsonElement _inputSchema;
    private static readonly JsonSerializerOptions _jsonOptions;

    static CreateCeilingPlanViewTool()
    {
        var schemaJson = """
            {
                "type": "object",
                "properties": {
                    "level_id": {
                        "type": "integer",
                        "description": "The element ID of the level to create the ceiling plan for"
                    },
                    "name": {
                        "type": "string",
    ...
    public string Name => "create_ceiling_plan_view";

    public string Description =>
        "Creates a new ceiling plan view for the specified level. " +
    ...
    public Task<ToolResult> ExecuteAsync(JsonElement input, UIApplication app, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Get ViewFamilyType for ceiling plans
            ...
            if (input.TryGetProperty("view_family_type_id", out var vftIdProp))
            {
                ...
                if (vft.ViewFamily != ViewFamily.CeilingPlan)
                    return Task.FromResult(ToolResult.Error($"ViewFamilyType {vft.Name} is not a CeilingPlan type."));
            }
            else
            {
                // Find default ceiling plan ViewFamilyType
            ...
            if (vft == null)
                return Task.FromResult(ToolResult.Error("No ceiling plan view family type found in the document."));
        ...
        {
            return Task.FromResult(ToolResult.Error($"Failed to create ceiling plan view: {ex.Message}"));
        }
    }
